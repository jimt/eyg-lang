...
pub fn simple_assignments_test() {
  let code =
    "let x = 1
  let y = {}"
  let #(acc, sections) = snippet.process_new([code])

  let snippet.State(_, referenced) = acc
  let snippet.Referenced(_, values, types) = referenced
...
pub fn known_reference_test() {
  ...
  let assert #([], Ok(#(ref, referenced))) =
    snippet.install_code(snippet.empty(), [], pre)

  let code = "let a = {foo: #foo}" |> string.replace("#foo", "#" <> ref)

  let #(acc, sections) = snippet.process([code], referenced)
